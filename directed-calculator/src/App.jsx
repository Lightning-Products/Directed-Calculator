  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", flexWrap: "wrap", maxWidth: 1400 }}>
        ...
        {/* LEFT PANEL */}
        <div style={{
          width: 350, minWidth: 310, padding: "0 28px 32px",
          flexShrink: 0,
        }}>
          <div style={{ background: BRAND.bgCard, borderRadius: 16, border: `1px solid ${BRAND.border}`, padding: "24px 24px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)", marginBottom: 16 }}>
            ...
            <Slider label="Total Apps" value={totalApps} onChange={setTotalApps}
              min={1} max={80} step={1} description="~6 brands, ~50 apps across all vendors" />
            <Slider label="Cost Per App / Year" value={vendorCostPerApp} onChange={setVendorCostPerApp}
              min={10000} max={40000} step={1000} format={fmt} description="Current vendor cost: $15K–$25K USD" />
            <Slider label="Vendor Cost Inflation" value={vendorInflation} onChange={setVendorInflation}
              min={0} max={25} step={1} suffix="% / yr" description="Costs already rising every few months" />
          </div>

          <div style={{ background: BRAND.bgCard, borderRadius: 16, border: `1px solid ${BRAND.border}`, padding: "24px 24px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)", marginBottom: 16 }}>
            <SectionHeader number="2" title="New Platform" subtitle="Shared multi-tenant architecture" />
            <Slider label="Platform Build Cost" value={platformBuildCost} onChange={setPlatformBuildCost}
              min={50000} max={800000} step={10000} format={fmt} description="One-off capex: architecture, backend, templates" />
            <Slider label="Cost Per New App" value={costPerNewApp} onChange={setCostPerNewApp}
              min={1000} max={20000} step={500} format={fmt} description="Incremental cost to onboard each new brand app" />
            <Slider label="Annual Infrastructure" value={annualInfraCost} onChange={setAnnualInfraCost}
              min={10000} max={200000} step={5000} format={fmt} description="Hosting, load balancing, Firebase, CI/CD" />
            <Slider label="Annual Support / Partner" value={annualSupportCost} onChange={setAnnualSupportCost}
              min={0} max={150000} step={5000} format={fmt} description="Lightning managed services retainer" />
          </div>
          ...
          <div style={{ background: BRAND.bgCard, borderRadius: 16, border: `1px solid ${BRAND.border}`, padding: "24px 24px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            ...
            <Slider label="Apps Onboarded / Quarter" value={appsPerQuarter} onChange={setAppsPerQuarter}
              min={1} max={15} step={1} description="Start with 1 brand, scale from there" />
            <Slider label="Projection Period" value={years} onChange={setYears}
              min={3} max={10} step={1} suffix=" years" />
          </div>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 420, padding: "0 32px 32px 16px" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
            <MetricCard
              label="Breakeven"
              value={breakeven ? `Year ${breakeven}` : "N/A"}
              sub={breakeven ? "Investment pays for itself" : "Adjust inputs"}
              accent={BRAND.purple}
            />
            <MetricCard
              label={`${years}-Year Savings`}
              value={totalSavings > 0 ? fmt(totalSavings) : `-${fmt(Math.abs(totalSavings))}`}
              sub="Cumulative vs vendor model"
              accent={totalSavings > 0 ? BRAND.green : BRAND.red}
            />
            <MetricCard
              label="ROI on Build"
              value={`${roi > 0 ? "+" : ""}${roi.toFixed(0)}%`}
              sub={`On ${fmt(platformBuildCost)} investment`}
              accent={roi > 0 ? BRAND.green : BRAND.red}
            />
            <MetricCard
              label="Current Annual Spend"
              value={fmt(totalApps * vendorCostPerApp)}
              sub={`${totalApps} apps × ${fmt(vendorCostPerApp)}`}
              accent={BRAND.yellow}
            />
          </div>
